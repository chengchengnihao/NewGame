...
#include "AnimTimelineClipboard_COPY.generated.h"
...
UCLASS(Transient)
class UAnimCurveBaseCopyObject_COPY : public UObject
{
	GENERATED_BODY()
	
public:

	UAnimCurveBaseCopyObject_COPY();
...
UCLASS(Transient)
class UFloatCurveCopyObject_COPY : public UAnimCurveBaseCopyObject_COPY
{
	GENERATED_BODY()
...
UCLASS(Transient)
class UTransformCurveCopyObject_COPY : public UAnimCurveBaseCopyObject_COPY
{
	GENERATED_BODY()
...
UCLASS(Transient)
class UVectorCurveCopyObject_COPY : public UAnimCurveBaseCopyObject_COPY
{
	GENERATED_BODY()
...
UCLASS(Transient)
class UAnimTimelineClipboardContent_COPY : public UObject
{
public:
	GENERATED_BODY()
	...
	UPROPERTY(Export)
	TArray<TObjectPtr<UAnimCurveBaseCopyObject_COPY>> Curves;
	...
	static UAnimTimelineClipboardContent_COPY* Create();
};

struct FAnimTimelineClipboardUtilities_COPY
{
	// Clipboard Content

	/** Copy anim timeline clipboard content to system's clipboard */
	static void CopyContentToClipboard(UAnimTimelineClipboardContent_COPY* Content);
	...
	static const UAnimTimelineClipboardContent_COPY* GetContentFromClipboard();
	 ...
	 */
	static bool CopySelectedTracksToClipboard(const TSet<TSharedRef<FAnimTimelineTrack_COPY>>& CurveTracks, UAnimTimelineClipboardContent_COPY* ClipboardContent);
	 ...
	 */
	static bool CanOverwriteSelectedCurveDataFromClipboard(const TSet<TSharedRef<FAnimTimelineTrack_COPY>>& SelectedTracks);
	 ...
	 */
	static void OverwriteSelectedCurveDataFromClipboard(const UAnimTimelineClipboardContent_COPY* ClipboardContent, const TSet<TSharedRef<FAnimTimelineTrack_COPY>>& CurveTracks, UAnimSequenceBase* InTargetSequence);
	 ...
	 * @param InTargetSequence Base Animation Sequence that will be modified
	 */
	static void OverwriteOrAddCurvesFromClipboardContent(const UAnimTimelineClipboardContent_COPY* ClipboardContent, UAnimSequenceBase* InTargetSequence);
};