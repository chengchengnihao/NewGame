...
#include "SAnimTimelineSplitterOverlay_COPY.h"
...
#define LOCTEXT_NAMESPACE "SAnimTimeline_COPY"

void SAnimTimeline_COPY::Construct(const FArguments& InArgs, const TSharedRef<FAnimModel_COPY>& InModel)
{
	TWeakPtr<FAnimModel_COPY> WeakModel = InModel;
	...
	{
		InModel->GetPreviewScene()->GetPreviewMeshComponent()->PreviewInstance->AddKeyCompleteDelegate(FSimpleDelegate::CreateSP(this, &SAnimTimeline_COPY::HandleKeyComplete));
	}
	...
	FTimeSliderArgs TimeSliderArgs;
	{
		...
		TimeSliderArgs.OnViewRangeChanged = FOnViewRangeChanged::CreateSP(&InModel.Get(), &FAnimModel_COPY::HandleViewRangeChanged);
		TimeSliderArgs.OnClampRangeChanged = FOnTimeRangeChanged::CreateSP(&InModel.Get(), &FAnimModel_COPY::HandleWorkingRangeChanged);
		...
		TimeSliderArgs.OnScrubPositionChanged = FOnScrubPositionChanged::CreateSP(this, &SAnimTimeline_COPY::HandleScrubPositionChanged);
	}

	TimeSliderController = MakeShareable(new FAnimTimeSliderController_COPY(TimeSliderArgs, InModel, SharedThis(this), SecondaryNumericTypeInterface));
	
	TSharedRef<FAnimTimeSliderController_COPY> TimeSliderControllerRef = TimeSliderController.ToSharedRef();
	...
	TrackArea = SNew(SAnimTrackArea_COPY, InModel, TimeSliderControllerRef);
	Outliner = SNew(SAnimOutliner_COPY, InModel, TrackArea.ToSharedRef())
	...
	{
		FillCoefficient_0.Bind(TAttribute<float>::FGetter::CreateSP(this, &SAnimTimeline_COPY::GetColumnFillCoefficient, 0));
		FillCoefficient_1.Bind(TAttribute<float>::FGetter::CreateSP(this, &SAnimTimeline_COPY::GetColumnFillCoefficient, 1));
	}
		...
		[
			...
			[
				...
				[
					...
					[
						...
						[
							...
							.OnTextChanged(this, &SAnimTimeline_COPY::OnOutlinerSearchChanged)
						]
						+SHorizontalBox::Slot()
						.VAlign(VAlign_Center)
						...
						[
							...
							[
								...
								.OnValueChanged(this, &SAnimTimeline_COPY::SetPlayTime)
								...
								.Delta(this, &SAnimTimeline_COPY::GetSpinboxDelta)
					...
					.HAlign(HAlign_Center)
					[
						SNew(SAnimTimelineTransportControls_COPY, InModel->GetPreviewScene(), InModel->GetAnimSequenceBase())
					]
					...
					.Padding(ResizeBarPadding)
					[
						SNew(SAnimTimelineOverlay_COPY, TimeSliderControllerRef)
				...
				[
					...
					SNew(SAnimTimelineSplitterOverlay_COPY)
					.Style(FAppStyle::Get(), "AnimTimeline.Outliner.Splitter")
					.Visibility(EVisibility::SelfHitTestInvisible)

					+ SSplitter::Slot()
					.Value(FillCoefficient_0)
					.OnSlotResized(SSplitter::FOnSlotResized::CreateSP(this, &SAnimTimeline_COPY::OnColumnFillCoefficientChanged, 0))
					[
						SNew(SSpacer)
					]
					...
					.OnSlotResized(SSplitter::FOnSlotResized::CreateSP(this, &SAnimTimeline_COPY::OnColumnFillCoefficientChanged, 1))
					[
						SNew(SSpacer)
					]
				]
...
FReply SAnimTimeline_COPY::OnMouseButtonUp(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	if(MouseEvent.GetEffectingButton() == EKeys::RightMouseButton)
	{
		...
		{
			MenuBuilder.AddMenuEntry(FAnimSequenceTimelineCommands_COPY::Get().SnapToFrames);
			MenuBuilder.AddMenuEntry(FAnimSequenceTimelineCommands_COPY::Get().SnapToNotifies);
			MenuBuilder.AddMenuEntry(FAnimSequenceTimelineCommands_COPY::Get().SnapToCompositeSegments);
			MenuBuilder.AddMenuEntry(FAnimSequenceTimelineCommands_COPY().SnapToMontageSections);
		}
		MenuBuilder.EndSection();
		...
		MenuBuilder.BeginSection("TimelineOptions", LOCTEXT("TimelineOptions", "Timeline Options") );
		{
			MenuBuilder.AddSubMenu(
				...
				FNewMenuDelegate::CreateLambda([](FMenuBuilder& InMenuBuilder)
				{
					...
					{
						InMenuBuilder.AddMenuEntry(FAnimSequenceTimelineCommands_COPY::Get().DisplaySeconds);
						InMenuBuilder.AddMenuEntry(FAnimSequenceTimelineCommands_COPY::Get().DisplayFrames);
					}
					InMenuBuilder.EndSection();

					InMenuBuilder.BeginSection("TimelineAdditional", LOCTEXT("TimelineAdditional", "Additional Display") );
					{
						InMenuBuilder.AddMenuEntry(FAnimSequenceTimelineCommands_COPY::Get().DisplayPercentage);
						InMenuBuilder.AddMenuEntry(FAnimSequenceTimelineCommands_COPY::Get().DisplaySecondaryFormat);
					}
					InMenuBuilder.EndSection();
				})
			);
		...
		if( AnimSequence )
		{
			...
			MenuBuilder.BeginSection("SequenceEditingContext", LOCTEXT("SequenceEditing", "Sequence Editing") );
			{
				...
				if (CurrentKeyIndex > 0)
				{
					...
					Action = FUIAction(FExecuteAction::CreateSP(this, &SAnimTimeline_COPY::OnCropAnimSequence, true, CorrectedFrameTime));
				...
				if (NextKeyIndex < NumKeys)
				{
					...
					Action = FUIAction(FExecuteAction::CreateSP(this, &SAnimTimeline_COPY::OnCropAnimSequence, false, NextFrameTime));
				...
				Action = FUIAction(FExecuteAction::CreateSP(this, &SAnimTimeline_COPY::OnInsertAnimSequence, true, CurrentKeyIndex));
				...
				Action = FUIAction(FExecuteAction::CreateSP(this, &SAnimTimeline_COPY::OnInsertAnimSequence, false, CurrentKeyIndex));
				...
				//Corrected frame time based on selected frame number
				Action = FUIAction(FExecuteAction::CreateSP(this, &SAnimTimeline_COPY::OnShowPopupOfAppendAnimation, WidgetPath, true));
				...
				Action = FUIAction(FExecuteAction::CreateSP(this, &SAnimTimeline_COPY::OnShowPopupOfAppendAnimation, WidgetPath, false));
				...
				MenuBuilder.AddMenuSeparator();
				//Menu - "ReZero"
				Action = FUIAction(FExecuteAction::CreateSP(this, &SAnimTimeline_COPY::OnReZeroAnimSequence, CurrentKeyIndex));
				...
				Action = FUIAction(FExecuteAction::CreateSP(this, &SAnimTimeline_COPY::OnReZeroAnimSequence, FrameNumberForCurrentTime));
...
void SAnimTimeline_COPY::OnCropAnimSequence( bool bFromStart, float CurrentTime )
{
...
void SAnimTimeline_COPY::OnAppendAnimSequence( bool bFromStart, int32 NumOfFrames )
{
	UAnimSingleNodeInstance* PreviewInstance = GetPreviewInstance();
...
void SAnimTimeline_COPY::OnInsertAnimSequence( bool bBefore, int32 CurrentFrame )
{
	UAnimSingleNodeInstance* PreviewInstance = GetPreviewInstance();
	if(PreviewInstance && PreviewInstance->GetCurrentAsset())
	{
...
void SAnimTimeline_COPY::OnReZeroAnimSequence(int32 FrameIndex)
{
	UAnimSingleNodeInstance* PreviewInstance = GetPreviewInstance();
	if(PreviewInstance)
	{
...
void SAnimTimeline_COPY::OnShowPopupOfAppendAnimation(FWidgetPath WidgetPath, bool bBegin)
{
	TSharedRef<STextEntryPopup> TextEntry =
		SNew(STextEntryPopup)
		.Label(LOCTEXT("AppendAnim_AskNumFrames", "Number of Frames to Append"))
		.OnTextCommitted(this, &SAnimTimeline_COPY::OnSequenceAppendedCalled, bBegin);
...
void SAnimTimeline_COPY::OnSequenceAppendedCalled(const FText & InNewGroupText, ETextCommit::Type CommitInfo, bool bBegin)
{
	// just a concern
...
TSharedRef<INumericTypeInterface<double>> SAnimTimeline_COPY::GetNumericTypeInterface() const
{
...
bool SAnimTimeline_COPY::GetGridMetrics(float PhysicalWidth, double& OutMajorInterval, int32& OutMinorDivisions) const
{
...
TSharedPtr<ITimeSliderController> SAnimTimeline_COPY::GetTimeSliderController() const 
{ 
	return TimeSliderController; 
}

void SAnimTimeline_COPY::OnOutlinerSearchChanged( const FText& Filter )
{
	FilterText = Filter;
...
void SAnimTimeline_COPY::OnColumnFillCoefficientChanged(float FillCoefficient, int32 ColumnIndex)
{
	ColumnFillCoefficients[ColumnIndex] = FillCoefficient;
}

void SAnimTimeline_COPY::HandleKeyComplete()
{
	Model.Pin()->RefreshTracks();
}

class UAnimSingleNodeInstance* SAnimTimeline_COPY::GetPreviewInstance() const
{
...
void SAnimTimeline_COPY::HandleScrubPositionChanged(FFrameTime NewScrubPosition, bool bIsScrubbing, bool bEvaluate) const
{
...
double SAnimTimeline_COPY::GetSpinboxDelta() const
{
...
void SAnimTimeline_COPY::SetPlayTime(double InFrameTime)
{
	if (UAnimSingleNodeInstance* PreviewInstance = GetPreviewInstance())
	{
		PreviewInstance->SetPlaying(false);
