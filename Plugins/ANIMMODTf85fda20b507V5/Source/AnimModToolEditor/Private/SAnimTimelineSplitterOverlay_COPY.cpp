...
void SAnimTimelineSplitterOverlay_COPY::Construct( const FArguments& InArgs )
{
	...
	{
		AddSlot()
		.Padding(TAttribute<FMargin>::Create(TAttribute<FMargin>::FGetter::CreateSP(this, &SAnimTimelineSplitterOverlay_COPY::GetSplitterHandlePadding, Index)))
		[
			SNew(SBox)
...
void SAnimTimelineSplitterOverlay_COPY::OnArrangeChildren( const FGeometry& AllottedGeometry, FArrangedChildren& ArrangedChildren ) const
{
...
FMargin SAnimTimelineSplitterOverlay_COPY::GetSplitterHandlePadding(int32 Index) const
{
...
FCursorReply SAnimTimelineSplitterOverlay_COPY::OnCursorQuery( const FGeometry& MyGeometry, const FPointerEvent& CursorEvent ) const
{
	return Splitter->OnCursorQuery(MyGeometry, CursorEvent);
}

FReply SAnimTimelineSplitterOverlay_COPY::OnMouseButtonDown( const FGeometry& MyGeometry, const FPointerEvent& MouseEvent )
{
...
void SAnimTimelineSplitterOverlay_COPY::OnMouseCaptureLost(const FCaptureLostEvent& CaptureLostEvent)
{
	SetVisibility(EVisibility::SelfHitTestInvisible);
	SOverlay::OnMouseCaptureLost(CaptureLostEvent);
}

FReply SAnimTimelineSplitterOverlay_COPY::OnMouseButtonUp( const FGeometry& MyGeometry, const FPointerEvent& MouseEvent )
{
...
FReply SAnimTimelineSplitterOverlay_COPY::OnMouseMove( const FGeometry& MyGeometry, const FPointerEvent& MouseEvent )
{
	return Splitter->OnMouseMove(MyGeometry, MouseEvent);
}

void SAnimTimelineSplitterOverlay_COPY::OnMouseLeave( const FPointerEvent& MouseEvent )
{
