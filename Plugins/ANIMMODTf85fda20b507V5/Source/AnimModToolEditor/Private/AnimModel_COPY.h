...
class FAnimModel_COPY : public TSharedFromThis<FAnimModel_COPY>, public FGCObject
{
public:
	FAnimModel_COPY(const TSharedRef<IPersonaPreviewScene>& InPreviewScene, const TSharedRef<IEditableSkeleton>& InEditableSkeleton, const TSharedRef<FUICommandList>& InCommandList);

	PRAGMA_DISABLE_DEPRECATION_WARNINGS
	virtual ~FAnimModel_COPY() {}
	...
	TArray<TSharedRef<FAnimTimelineTrack_COPY>>& GetRootTracks() 
	{
		PRAGMA_DISABLE_DEPRECATION_WARNINGS
		return RootTracks;
		PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	/** Get the root tracks representing the tree */
	const TArray<TSharedRef<FAnimTimelineTrack_COPY>>& GetRootTracks() const 
	{
		PRAGMA_DISABLE_DEPRECATION_WARNINGS
		return RootTracks;
		PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	const TArray<TSharedRef<FAnimTimelineTrack_COPY>>& GetAllRootTracks() const
	{
		PRAGMA_DISABLE_DEPRECATION_WARNINGS
		return RootTracks;
		PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	DECLARE_EVENT(FAnimModel_COPY, FOnTracksChanged)
	FOnTracksChanged& OnTracksChanged() { return OnTracksChangedDelegate; }

	/** Delegate fired when objects have been selected */
	DECLARE_EVENT_OneParam(FAnimModel_COPY, FOnHandleObjectsSelected, const TArray<UObject*>& /*InObjects*/)
	...
	bool IsTrackSelected(const TSharedRef<FAnimTimelineTrack_COPY>& InTrack) const;

	/** Clear all track selection */
	void ClearTrackSelection();

	/** Set whether a track is selected */
	void SetTrackSelected(const TSharedRef<FAnimTimelineTrack_COPY>& InTrack, bool bIsSelected);
	...
	void ForEachRootTrack(TFunctionRef<void(FAnimTimelineTrack_COPY&)> InFunction);

protected:
	/** Adds a track to the root tracks collection */
	void AddRootTrack(TSharedRef<FAnimTimelineTrack_COPY> InTrack);
...
protected:	
	...
	TArray<TSharedRef<FAnimTimelineTrack_COPY>> RootTracks;

	/** Tracks that are selected */
	TSet<TSharedRef<FAnimTimelineTrack_COPY>> SelectedTracks;
	...
	FEditorObjectTracker_COPY EditorObjectTracker;
	...
	struct FSnapType
	{
		FSnapType(const FName& InType, const FText& InDisplayName, TFunction<double(const FAnimModel_COPY&, double)> InSnapFunction = nullptr)
			: Type(InType)
		...
		TFunction<double(const FAnimModel_COPY&, double)> SnapFunction;
