...
ANIMTIMELINE_IMPLEMENT_TRACK(FAnimTimelineTrack_CompoundCurve_COPY);
ANIMTIMELINE_IMPLEMENT_TRACK(FAnimTimelineTrack_FloatCurveWithDisplayName_COPY);

FAnimTimelineTrack_FloatCurveWithDisplayName_COPY::FAnimTimelineTrack_FloatCurveWithDisplayName_COPY(const FFloatCurve* InCurve, FText InDisplayName, const TSharedRef<FAnimModel_COPY>& InModel)
	: FAnimTimelineTrack_FloatCurve_COPY(InCurve, InModel)
{
	DisplayName = InDisplayName;
}

FText FAnimTimelineTrack_FloatCurveWithDisplayName_COPY::GetLabel() const
{
	// FAnimTimelineTrack_FloatCurve_COPY forces the label to its underlying curve name
	// We want to use the shorter alias for it
	return FAnimTimelineTrack_COPY::GetLabel();
}

bool FAnimTimelineTrack_FloatCurveWithDisplayName_COPY::ShowCurves() const
{
	// Always show curves even if expanded
	return true;
}

TArray<const FRichCurve*> FAnimTimelineTrack_CompoundCurve_COPY::ToRichCurves(TArrayView<FFloatCurve const* const> InCurves)
{
...
FAnimTimelineTrack_CompoundCurve_COPY::FAnimTimelineTrack_CompoundCurve_COPY(TArray<const FFloatCurve*> InCurves, const FText& InCurveName, const FText& InFullCurveName, const FLinearColor& InBackgroundColor, const TSharedRef<FAnimModel_COPY>& InModel)
	: FAnimTimelineTrack_Curve_COPY(ToRichCurves(InCurves), InCurveName, InFullCurveName, FLinearColor{}, InBackgroundColor, InModel)
	, Curves(MoveTemp(InCurves))
{
	OuterCurveIndex = 0;
	Algo::Transform(Curves, CurveNames, [](const FFloatCurve* Curve) { return Curve->Name_DEPRECATED; });
}

/** FAnimTimelineTrack_Curve_COPY interface */
FLinearColor FAnimTimelineTrack_CompoundCurve_COPY::GetCurveColor(int32 InCurveIndex) const
{
	return Curves[InCurveIndex]->Color;
}

FText FAnimTimelineTrack_CompoundCurve_COPY::GetFullCurveName(int32 InCurveIndex) const
{
	return FText::FromName(Curves[InCurveIndex]->Name_DEPRECATED.DisplayName);
}

bool FAnimTimelineTrack_CompoundCurve_COPY::CanEditCurve(int32 InCurveIndex) const
{
	return true;
}

void FAnimTimelineTrack_CompoundCurve_COPY::GetCurveEditInfo(int32 InCurveIndex, FSmartName& OutName, ERawCurveTrackTypes& OutType, int32& OutCurveIndex) const
{
	OutName = Curves[InCurveIndex]->Name_DEPRECATED;
	OutType = ERawCurveTrackTypes::RCT_Float;
	OutCurveIndex = 0;
}

struct FAnimTimelineTrack_CompoundCurve_COPY::FCurveGroup
{
	...
	static FCurveGroup MakeCurveGroup(TArrayView<const FFloatCurve> FloatCurves, FStringView Delimiters)
	{
		...
		{
			FString CurveNameStr = FloatCurve.Name_DEPRECATED.DisplayName.ToString();
	...
	void AddCurvesToTrack(const TSharedRef<FAnimModel_COPY>& InModel, FAnimTimelineTrack_COPY* ParentTrack)
	{
		TSharedPtr<FAnimTimelineTrack_COPY> CurveTrack;
		...
		if (!IsRoot())
		{
			if (Curve != nullptr)
			{
				...
				CurveTrack = MakeShared<FAnimTimelineTrack_FloatCurveWithDisplayName_COPY>(Curve, FText::FromStringView(Name()), InModel);
			}
			else
			{
				...
				CurveTrack = MakeShared<FAnimTimelineTrack_CompoundCurve_COPY>(MoveTemp(CurvesInGroup), FText::FromStringView(Name()), FText::FromString(FullName), CurveBackgroundColor, InModel);
...
void FAnimTimelineTrack_CompoundCurve_COPY::AddGroupedCurveTracks(TArrayView<const FFloatCurve> FloatCurves, FAnimTimelineTrack_COPY& ParentTrack, const TSharedRef<FAnimModel_COPY>& InModel, FStringView Delimiters)
{
