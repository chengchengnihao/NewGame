...
#include "SAnimNotifyPanel_COPY.h"
...
const float NotifyHeight = FAnimTimelineTrack_NotifiesPanel_COPY::NotificationTrackHeight;
...
private:
	...
	TWeakPtr<SAnimNotifyPanel_COPY>			AnimPanelPtr;
...
public:
	...
	{}
	SLATE_ARGUMENT( int32, TrackIndex )
	SLATE_ARGUMENT( TSharedPtr<SAnimNotifyPanel_COPY>, AnimNotifyPanel)
	SLATE_ARGUMENT( class UAnimSequenceBase*, Sequence )
...
FVector2D SAnimNotifyTrack::ComputeDesiredSize( float ) const
{
	FVector2D Size;
	Size.X = 200;
	Size.Y = FAnimTimelineTrack_NotifiesPanel_COPY::NotificationTrackHeight;
	return Size;
}
...
void SAnimNotifyTrack::Update()
{
	...
	if ( AnimNotifies.Num() > 0 )
	{
		...
		SAnimTimingPanel_COPY::GetTimingRelevantElements(Sequence, TimingElements);
...
void SNotifyEdTrack::Construct(const FArguments& InArgs)
{
	...
	TSharedRef<SAnimNotifyPanel_COPY> PanelRef = InArgs._AnimNotifyPanel.ToSharedRef();
	...
	[
			...
			[
				...
				.OnRequestTrackPan(FPanTrackRequest::CreateSP(PanelRef, &SAnimNotifyPanel_COPY::PanInputViewRange))
				.OnRequestOffsetRefresh(InArgs._OnRequestRefreshOffsets)
				.OnDeleteNotify(InArgs._OnDeleteNotify)
				.OnGetIsAnimNotifySelectionValidForReplacement(PanelRef, &SAnimNotifyPanel_COPY::IsNotifySelectionValidForReplacement)
				.OnReplaceSelectedWithNotify(PanelRef, &SAnimNotifyPanel_COPY::OnReplaceSelectedWithNotify)
				.OnReplaceSelectedWithBlueprintNotify(PanelRef, &SAnimNotifyPanel_COPY::OnReplaceSelectedWithNotifyBlueprint)
				.OnDeselectAllNotifies(InArgs._OnDeselectAllNotifies)
				.OnCopyNodes(InArgs._OnCopyNodes)
				.OnPasteNodes(InArgs._OnPasteNodes)
				.OnSetInputViewRange(InArgs._OnSetInputViewRange)
...
//////////////////////////////////////////////////////////////////////////
// SAnimNotifyPanel_COPY

void SAnimNotifyPanel_COPY::Construct(const FArguments& InArgs, const TSharedRef<FAnimModel_COPY>& InModel)
{
	SAnimTrackPanel_COPY::Construct( SAnimTrackPanel_COPY::FArguments()
	...
	InModel->OnHandleObjectsSelected().AddSP(this, &SAnimNotifyPanel_COPY::HandleObjectsSelected);

	FAnimNotifyPanelCommands::Register();
	BindCommands();

	Sequence->RegisterOnNotifyChanged(UAnimSequenceBase::FOnNotifyChanged::CreateSP(this, &SAnimNotifyPanel_COPY::RequestRefresh ));

	InModel->GetEditableSkeleton()->RegisterOnNotifiesChanged(FSimpleDelegate::CreateSP(this, &SAnimNotifyPanel_COPY::RequestRefresh));
	InModel->OnTracksChanged().Add(FSimpleDelegate::CreateSP(this, &SAnimNotifyPanel_COPY::RequestRefresh));
	...
	OnPropertyChangedHandle = FCoreUObjectDelegates::FOnObjectPropertyChanged::FDelegate::CreateSP(this, &SAnimNotifyPanel_COPY::OnPropertyChanged);
...
SAnimNotifyPanel_COPY::~SAnimNotifyPanel_COPY()
{
...
FName SAnimNotifyPanel_COPY::GetNewTrackName() const
{
...
FReply SAnimNotifyPanel_COPY::InsertTrack(int32 TrackIndexToInsert)
{
...
FReply SAnimNotifyPanel_COPY::AddTrack()
{
	FAnimNotifyTrack NewItem;
	NewItem.TrackName = GetNewTrackName();
	NewItem.TrackColor = FLinearColor::White;
...
FReply SAnimNotifyPanel_COPY::DeleteTrack(int32 TrackIndexToDelete)
{
...
bool SAnimNotifyPanel_COPY::CanDeleteTrack(int32 TrackIndexToDelete)
{
...
void SAnimNotifyPanel_COPY::OnCommitTrackName(const FText& InText, ETextCommit::Type CommitInfo, int32 TrackIndexToName)
{
...
void SAnimNotifyPanel_COPY::Update()
{
	if(!bIsUpdating)
	{
...
void SAnimNotifyPanel_COPY::RequestUpdate()
{
	bUpdateRequested = true;
}

void SAnimNotifyPanel_COPY::RequestRefresh()
{
...
{
	FScopedSavedNotifySelection(SAnimNotifyPanel_COPY& InPanel)
		: Panel(InPanel)
	{
	...
	SAnimNotifyPanel_COPY& Panel;
	TSet<FGuid> SelectedNodeGuids;
};

void SAnimNotifyPanel_COPY::RefreshNotifyTracks()
{
	check (Sequence);

	{
		...
		for(int32 TrackIndex = 0; TrackIndex < Sequence->AnimNotifyTracks.Num(); TrackIndex++)
		{
			...
			[
				...
				.OnGetDraggedNodePos(this, &SAnimNotifyPanel_COPY::CalculateDraggedNodePos)
				.OnUpdatePanel(this, &SAnimNotifyPanel_COPY::RequestUpdate)
				.OnGetNotifyBlueprintData(this, &SAnimNotifyPanel_COPY::OnGetNotifyBlueprintData, &NotifyClassNames)
				.OnGetNotifyStateBlueprintData(this, &SAnimNotifyPanel_COPY::OnGetNotifyBlueprintData, &NotifyStateClassNames)
				.OnGetNotifyNativeClasses(this, &SAnimNotifyPanel_COPY::OnGetNativeNotifyData, UAnimNotify::StaticClass(), &NotifyClassNames)
				.OnGetNotifyStateNativeClasses(this, &SAnimNotifyPanel_COPY::OnGetNativeNotifyData, UAnimNotifyState::StaticClass(), &NotifyStateClassNames)
				.OnSelectionChanged(this, &SAnimNotifyPanel_COPY::OnTrackSelectionChanged)
				.OnNodeDragStarted(this, &SAnimNotifyPanel_COPY::OnNotifyNodeDragStarted)
				.OnNotifyStateHandleBeingDragged(OnNotifyStateHandleBeingDragged)
				.OnSnapPosition(OnSnapPosition)
				.OnRequestRefreshOffsets(OnRequestRefreshOffsets)
				.OnDeleteNotify(this, &SAnimNotifyPanel_COPY::DeleteSelectedNodeObjects)
				.OnDeselectAllNotifies(this, &SAnimNotifyPanel_COPY::DeselectAllNotifies)
				.OnCopyNodes(this, &SAnimNotifyPanel_COPY::CopySelectedNodesToClipboard)
				.OnPasteNodes(this, &SAnimNotifyPanel_COPY::OnPasteNodes)
				.OnSetInputViewRange(this, &SAnimNotifyPanel_COPY::InputViewRangeChanged)
...
float SAnimNotifyPanel_COPY::CalculateDraggedNodePos() const
{
	return CurrentDragXPosition;
}

FReply SAnimNotifyPanel_COPY::OnNotifyNodeDragStarted(TArray<TSharedPtr<SAnimNotifyNode>> NotifyNodes, TSharedRef<SWidget> Decorator, const FVector2D& ScreenCursorPos, const FVector2D& ScreenNodePosition, const bool bDragOnMarker)
{
	...
	FPanTrackRequest PanRequestDelegate = FPanTrackRequest::CreateSP(this, &SAnimNotifyPanel_COPY::PanInputViewRange);
	FOnUpdatePanel UpdateDelegate = FOnUpdatePanel::CreateSP(this, &SAnimNotifyPanel_COPY::RequestUpdate);
...
float SAnimNotifyPanel_COPY::GetSequenceLength() const
{
	return Sequence->GetPlayLength();
}

void SAnimNotifyPanel_COPY::PostUndo( bool bSuccess )
{
	if(Sequence != NULL)
	{
		Sequence->RefreshCacheData();
...
void SAnimNotifyPanel_COPY::PostRedo( bool bSuccess )
{
	if(Sequence != NULL)
	{
		Sequence->RefreshCacheData();
...
void SAnimNotifyPanel_COPY::OnDeletePressed()
{
...
void SAnimNotifyPanel_COPY::DeleteSelectedNodeObjects()
{
...
void SAnimNotifyPanel_COPY::SetSequence(class UAnimSequenceBase*	InSequence)
{
...
void SAnimNotifyPanel_COPY::OnTrackSelectionChanged()
{
	if(!bIsSelecting)
	{
		...
		for(int32 TrackIdx = 0 ; TrackIdx < NotifyAnimTracks.Num() ; ++TrackIdx)
		{
			...
			for(int32 Idx : TrackIndices)
			{
				...
				if (FAnimNotifyEvent* NotifyEvent = NodeObjectInterface->GetNotifyEvent())
				{
					...
					NewNotifyObject->InitFromAnim(Sequence, FOnAnimObjectChange::CreateSP(this, &SAnimNotifyPanel_COPY::OnNotifyObjectChanged));
...
void SAnimNotifyPanel_COPY::DeselectAllNotifies()
{
	if(!bIsSelecting)
	{
		TGuardValue<bool> GuardValue(bIsSelecting, true);
...
void SAnimNotifyPanel_COPY::CopySelectedNodesToClipboard() const
{
...
bool SAnimNotifyPanel_COPY::IsNotifySelectionValidForReplacement()
{
	// Grab the selected events
	TArray<INodeObjectInterface*> SelectedNodes;
	for (TSharedPtr<SAnimNotifyTrack> Track : NotifyAnimTracks)
...
void SAnimNotifyPanel_COPY::OnReplaceSelectedWithNotify(FString NewNotifyName, UClass* NewNotifyClass)
{
	TArray<INodeObjectInterface*> SelectedNodes;
	for (TSharedPtr<SAnimNotifyTrack> Track : NotifyAnimTracks)
	{
...
void SAnimNotifyPanel_COPY::OnReplaceSelectedWithNotifyBlueprint(FString NewBlueprintNotifyName, FString NewBlueprintNotifyClass)
{
...
void SAnimNotifyPanel_COPY::OnPasteNodes(SAnimNotifyTrack* RequestTrack, float ClickTime, ENotifyPasteMode::Type PasteMode, ENotifyPasteMultipleMode::Type MultiplePasteType)
{
...
void SAnimNotifyPanel_COPY::OnPropertyChanged(UObject* ChangedObject, FPropertyChangedEvent& PropertyEvent)
{
...
void SAnimNotifyPanel_COPY::BindCommands()
{
	...
	CommandList->MapAction(
		...
		FExecuteAction::CreateSP(this, &SAnimNotifyPanel_COPY::OnDeletePressed));

	CommandList->MapAction(
		Commands.CopyNotifies,
		FExecuteAction::CreateSP(this, &SAnimNotifyPanel_COPY::CopySelectedNodesToClipboard));

	CommandList->MapAction(
		Commands.PasteNotifies,
		FExecuteAction::CreateSP(this, &SAnimNotifyPanel_COPY::OnPasteNodes, (SAnimNotifyTrack*)nullptr, -1.0f, ENotifyPasteMode::MousePosition, ENotifyPasteMultipleMode::Absolute));
}

FReply SAnimNotifyPanel_COPY::OnKeyDown(const FGeometry& MyGeometry, const FKeyEvent& InKeyEvent)
{
...
FReply SAnimNotifyPanel_COPY::OnMouseButtonDown(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	SAnimTrackPanel_COPY::OnMouseButtonDown(MyGeometry, MouseEvent);
...
FReply SAnimNotifyPanel_COPY::OnMouseButtonUp(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	if(Marquee.bActive)
	{
		OnTrackSelectionChanged();
	...
	return SAnimTrackPanel_COPY::OnMouseButtonUp(MyGeometry, MouseEvent);
}

FReply SAnimNotifyPanel_COPY::OnMouseMove(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	FReply BaseReply = SAnimTrackPanel_COPY::OnMouseMove(MyGeometry, MouseEvent);
...
int32 SAnimNotifyPanel_COPY::OnPaint(const FPaintArgs& Args, const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FWidgetStyle& InWidgetStyle, bool bParentEnabled) const
{
	LayerId = SAnimTrackPanel_COPY::OnPaint(Args, AllottedGeometry, MyCullingRect, OutDrawElements, LayerId, InWidgetStyle, bParentEnabled);
...
void SAnimNotifyPanel_COPY::Tick( const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime )
{
	if(bUpdateRequested)
	{
		Update();
...
void SAnimNotifyPanel_COPY::RefreshMarqueeSelectedNodes(const FGeometry& PanelGeo)
{
	if(Marquee.IsValid())
	{
...
FReply SAnimNotifyPanel_COPY::OnDragDetected(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	Marquee.bActive = true;
	return FReply::Handled().CaptureMouse(SharedThis(this));
}

void SAnimNotifyPanel_COPY::OnFocusLost(const FFocusEvent& InFocusEvent)
{
	if(Marquee.bActive)
	{
		OnTrackSelectionChanged();
...
void SAnimNotifyPanel_COPY::PopulateNotifyBlueprintClasses(TArray<FString>& InOutAllowedClasses)
{
	TArray<FAssetData> TempArray;
	OnGetNotifyBlueprintData(TempArray, &InOutAllowedClasses);
}

void SAnimNotifyPanel_COPY::OnGetNotifyBlueprintData(TArray<FAssetData>& OutNotifyData, TArray<FString>* InOutAllowedClassNames)
{
...
void SAnimNotifyPanel_COPY::OnGetNativeNotifyData(TArray<UClass*>& OutClasses, UClass* NotifyOutermost, TArray<FString>* OutAllowedBlueprintClassNames)
{
...
void SAnimNotifyPanel_COPY::OnNotifyObjectChanged(UObject* EditorBaseObj, bool bRebuild)
{
...
void SAnimNotifyPanel_COPY::OnNotifyTrackScrolled(float InScrollOffsetFraction)
{
...
void SAnimNotifyPanel_COPY::InputViewRangeChanged(float ViewMin, float ViewMax)
{
	...
	SAnimTrackPanel_COPY::InputViewRangeChanged(ViewMin, ViewMax);
}

void SAnimNotifyPanel_COPY::HandleObjectsSelected(const TArray<UObject*>& InObjects)
{
	if(!bIsSelecting)
	{
		DeselectAllNotifies();
