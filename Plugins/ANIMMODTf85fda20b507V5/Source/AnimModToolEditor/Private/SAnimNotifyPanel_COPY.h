...
public:
	FAnimNotifyPanelCommands()
		: TCommands<FAnimNotifyPanelCommands>("AnimNotifyPanel (AnimModTools)", NSLOCTEXT("Contexts", "AnimNotifyPanel_AnimModTools", "Anim Notify Panel (AnimModTools)"), NAME_None, FAppStyle::GetAppStyleSetName())
	{
...
class SAnimNotifyPanel_COPY: public SAnimTrackPanel_COPY, public FEditorUndoClient
{
public:
	SLATE_BEGIN_ARGS( SAnimNotifyPanel_COPY )
		: _Sequence()
		, _CurrentPosition()
		, _ViewInputMin()
		, _ViewInputMax()
	...
	void Construct(const FArguments& InArgs, const TSharedRef<FAnimModel_COPY>& InModel);
	virtual ~SAnimNotifyPanel_COPY();
