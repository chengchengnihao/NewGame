...
void AAuraPlayerController::SetupInputComponent()
{
	...
	EnhancedInputComponent->BindAction(ControllerYaw,ETriggerEvent::Triggered,this,&AAuraPlayerController::Action_ControllerYaw);
	EnhancedInputComponent->BindAction(ControllerPitch,ETriggerEvent::Triggered,this,&AAuraPlayerController::Action_ControllerPitch);
...
void AAuraPlayerController::Action_ControllerYaw(const FInputActionValue& Value)
{
	float AValue = Value.Get<float>();
	if (APawn* ControlledPawn = GetPawn())
	{
		ControlledPawn->AddControllerYawInput(AValue);
	}
}

void AAuraPlayerController::Action_ControllerPitch(const FInputActionValue& Value)
{
	float BValue = Value.Get<float>();
	UE_LOG(LogTemp, Log, TEXT("Pitch Value: %f"), BValue);
	if (APawn* ControlledPawn = GetPawn())
	{
		ControlledPawn->AddControllerPitchInput(BValue);
	}
}
